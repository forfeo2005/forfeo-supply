...
const Dashboard = () => {
  ...
  const fetchData = async (userId) => {
    ...
    // Produits
    const { data: productsData } = await supabase.from('products').select('*').eq('supplier_id', userId).order('created_at', { ascending: false });
  ...
  const generateInvoice = (order) => {
    ...
    doc.setFont("helvetica", "bold"); doc.text("TOTAL (Taxes incluses):", 100, y); doc.text(total.toFixed(2) + " CAD", 160, y);
    doc.save(`Facture_${order.id.slice(0,8)}.pdf`);
  };

  // --- AJOUT PRODUIT (CORRIGÉ) ---
  const handleAddProduct = async (e) => {
    e.preventDefault();
    const producerName = profile.company_name || user.email || "Fournisseur"; // Fix null error
    ...
    else { setShowAddForm(false); fetchData(user.id); }
  };
  ...
  const deleteProduct = async (id) => {
    ...
    if(error) alert("Erreur suppression: " + error.message); else fetchData(user.id);
  };
  ...
  const handleSaveProfile = async (e) => {
    ...
    alert("Sauvegardé !"); setSavingProfile(false);
  };
        ...
        {activeTab === 'overview' && (
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              <StatCard title="Commandes" value={orders.length} icon={<Icons.Cart />} color="bg-blue-100 text-blue-600" />
              <StatCard title="Produits" value={products.length} icon={<Icons.Package />} color="bg-emerald-100 text-emerald-600" />
            </div>
          </div>
        )}
        ...
        {activeTab === 'products' && (
          <div className="max-w-6xl mx-auto">
            <div className="flex justify-between items-center mb-8">
              ...
              <button onClick={() => setShowAddForm(true)} className="bg-emerald-600 text-white px-4 py-2 rounded-lg flex gap-2"><Icons.Plus /> Ajouter</button>
            </div>
            {showAddForm && (
              <div className="bg-white p-6 rounded-xl shadow-sm mb-8">
                <form onSubmit={handleAddProduct} className="grid md:grid-cols-2 gap-4">
                  ...
                  <select className="p-3 border rounded" value={newProduct.category} onChange={e => setNewProduct({...newProduct, category: e.target.value})}><option value="Alimentaire">Alimentaire</option><option value="Bureau">Bureau</option><option value="Équipement">Équipement</option></select>
            ...
            <div className="bg-white rounded-xl shadow-sm overflow-hidden">
              <table className="w-full text-left">
                <thead className="bg-slate-50 text-xs uppercase"><tr><th className="p-4">Produit</th><th className="p-4">Prix</th><th className="p-4">Action</th></tr></thead>
                <tbody>{products.map(p => (<tr key={p.id} className="border-t"><td className="p-4 font-bold">{p.name}</td><td className="p-4">{p.price}$</td><td className="p-4"><button onClick={() => deleteProduct(p.id)} className="text-red-500 hover:text-red-700"><Icons.Trash /></button></td></tr>))}</tbody>
              </table>
            </div>
          </div>
        )}
            ...
            <div className="space-y-4">
              {orders.map(o => (
                <div key={o.id} className="bg-white p-6 rounded-xl shadow-sm border flex flex-col md:flex-row justify-between items-center gap-4">
                  <div><span className="font-bold">#{o.id.slice(0,8)}</span> - {o.buyer?.company_name}</div>
                  <div className="flex gap-2">
                    <button onClick={() => generateInvoice(o)} className="bg-slate-100 px-3 py-2 rounded text-sm font-bold">📄 Facture</button>
                    {o.status === 'confirmed' && <button onClick={() => handlePrintLabel(o)} className="bg-slate-800 text-white px-3 py-2 rounded text-sm font-bold">🖨️ Étiquette</button>}
                  </div>
                </div>
              ))}
            </div>
